...
def _operator_chat_enabled() -> bool:
    """Gates the interactive "continue operating / type done in chat" loop
    below. This trimmed port doesn't wire up a chat UI (see routers/drafts.py),
    so leaving this loop unconditional would make a fill request hang
    forever waiting for a chat message that will never arrive. Off by
    default; set APPLICATION_ENABLE_OPERATOR_CHAT=1 if you build that UI
    back in and want the multi-cycle operator session behavior.
    """
    return os.environ.get("APPLICATION_ENABLE_OPERATOR_CHAT", "").strip().lower() in {"1", "true", "yes"}
...
async def _run_submission(
    ...
    *,
    ...
    try:
        ...
        if use_visible_browser and _operator_chat_enabled():
