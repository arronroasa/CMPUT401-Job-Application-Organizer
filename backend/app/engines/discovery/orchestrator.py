...
def _load_profile(db_conn: sqlite3.Connection, user_profile: dict[str, Any] | None) -> dict[str, Any] | None:
    """Best-effort profile lookup, used only to enrich ranking (skills,
    experience). A search no longer requires a profile to exist — see
    run_discovery's `query` argument, which lets the caller specify what to
    search for directly instead of relying on resume-derived role matches.
    """
    if user_profile is not None:
        return user_profile
    # Most recently touched profile, regardless of id — resume uploads via
    # /profile/resume create a fresh profile id each time rather than a
    # fixed 'local' row, so picking the newest one is what actually reflects
    # "whatever resume was uploaded last".
    row = db_conn.execute(
...
def _build_queries_from_profile(profile: dict[str, Any]) -> list[str]:
...
async def run_discovery(
    ...
    *,
    ...
    """Run a job search against public sources.

    `query` lets the caller specify exactly what to search for (e.g.
    "backend engineer intern"), the same way you'd type a search into the
    original careersavers UI — this is the primary path. When `query` is
    omitted, discovery falls back to generating queries from the active
    profile's resume-derived role interests, same as before.

    A profile is no longer required to run a search at all; when present
    (most recently uploaded resume) it's used only to enrich ranking with
    skill/experience match scores.
    """
    ...
    try:
        profile = _load_profile(db_conn, user_profile) or {}

        cleaned_query = (query or "").strip()
        if cleaned_query:
            queries = generate_queries(cleaned_query, (location or "remote").strip() or "remote", remote)
            # Give ranking something to compare job titles against even
            # when there's no resume-derived profile yet.
            if not _parse_json_array(profile.get("role_interests_json")):
                profile = {**profile, "role_interests_json": json.dumps([{"title": cleaned_query}])}
        else:
            queries = _build_queries_from_profile(profile)

        if not queries:
            logger.info("Discovery skipped: no search query and no role interests available")
            ...
            db_conn.execute(
                ...
                WHERE id = ?
                """,
                (completed_at, "skipped_no_query", run_id),
            )
            db_conn.commit()
            return {"run_id": run_id, "jobs_found": 0, "jobs_new": 0, "status": "skipped_no_query"}
