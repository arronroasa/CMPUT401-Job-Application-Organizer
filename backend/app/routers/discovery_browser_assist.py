"""Browser-assisted job discovery — drives a real, visible Chrome browser
(via CDP, the same debugging protocol Chrome DevTools uses) to search
LinkedIn/Indeed interactively using YOUR logged-in session, since those
sites block headless/API scraping. This needs a Chrome window running on
the operator's machine with --remote-debugging-port enabled (see
/discovery/browser-status for exact commands) — it's a separate mode
from the fully-automated bulk discovery in discovery.py (which only hits
public sources like Greenhouse and doesn't need a browser at all).

Mounted on the same "/discovery" prefix as discovery.py — FastAPI merges
routes from multiple routers sharing a prefix, so this purely adds
endpoints alongside the existing ones without touching that file.
"""
...
router = APIRouter(prefix="/discovery", tags=["discovery-browser-assist"])
